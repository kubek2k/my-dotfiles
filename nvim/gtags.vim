...
if !exists("g:Gtags_Result")
    let g:Gtags_Result = "cscope"
endif
if !exists("g:Gtags_Efm")
    let Gtags_Efm = "%c%*\\s%f%*\\s%*\\S%*\\s%l%*\\s%m"
endif
...
function! s:getGtagsCscopeColumn(line)
    let l:parts = split(a:line, " ", 1)
    let l:actualHit = join(l:parts[2:], " ")   
    echom l:actualHit
    let l:symbol = l:parts[1]
    let l:column = match(l:actualHit, l:symbol)
    echom l:column
    return l:column
endfunction
...
function! s:ExecLoad(option, long_option, pattern, flags)
    ...
    let l:result = map(l:result, 's:getGtagsCscopeColumn(v:val) . "\t" . v:val')
